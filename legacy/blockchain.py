...
class BlockChain:
    ...
    def sign_transaction(self, transaction):
        if "message" in transaction:
            ...
            transaction["hash"] = sha256(
                bytes.fromhex(transaction["from"]) + bytes.fromhex(transaction["to"]) + transaction[
    ...
    def generate_block(self):
        if len(self.chain):
            ...
            transaction = {
                "to": self.public_key,
                "message": "genesis block, y="+str(self.chameleon["y"])
            }
    ...
    def revoke_transaction(self, merkle_tree, tran_hash):
        ...
        # 如果撤销之后交易数为0，为了保持区块结构，添加一个 NULL 交易补充
            ...
            return True, hashTool.merkel_tree([self.sign_transaction({"to": self.public_key, "message": "NULL"})])
    ...
    def verify_transaction(self, transaction):
        try:
            ...
            # 验证哈希
            current_hash = sha256(
                bytes.fromhex(t_from) + bytes.fromhex(t_to) + t_message.encode() + t_signature.encode()).hexdigest()
